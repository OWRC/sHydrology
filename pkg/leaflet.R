...
output$map <- renderLeaflet({
  ...
  leaflet(ormgp.bound) %>%
    ...
    addTiles("https://tile.oakridgeswater.ca/basemap/{z}/{x}/{y}", group = "ORMGP basemap", options = providerTileOptions(attribution=" © Oak Ridges Moraine Groundwater Program", maxNativeZoom = 17)) %>%
    addTiles("https://tile.oakridgeswater.ca/lidar24/{z}/{x}/{y}", group = "High definition DEM", options = providerTileOptions(attribution=" © Oak Ridges Moraine Groundwater Program", maxNativeZoom = 18)) %>%
